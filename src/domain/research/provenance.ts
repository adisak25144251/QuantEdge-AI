...
export function extractLatestSecFact(
  ...
  const latest = candidates.sort((a, b) => String(b.filed).localeCompare(String(a.filed)))[0];
  ...
  const ageMs = Date.now() - Date.parse(String(latest.filed));
  ...
  return {
    ...
    status: Number.isFinite(ageMs) && ageMs <= 550 * 86_400_000 ? 'VERIFIED' : 'STALE'
  };
}
